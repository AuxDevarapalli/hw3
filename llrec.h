...
Node* llfilter(Node* head, Comp pred)
{
    ...
    //*********************************************
    //safe check for null head
  if(head==NULL){
    return NULL;
  }
  //uses same logic as llpivot for recursive part
  Node* tempy=head;
  Node* restL=head->next;
  //head=restL;
  //recursive call finds the filtered tail
  Node* res=llfilter(restL,pred);
  if(pred(tempy->val)){//this part removes that node thenreturn filterde tail
    //Node* temp=head;
    head=head->next;
    //temp->next=head;
    delete tempy;
  ...
  else{// this part returns head after connecting tail
    tempy->next=res;
    return tempy;
  }
